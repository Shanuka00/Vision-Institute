...
function DashboardTe() {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        const coursesResponse = await api.get('/loadTeCourses', { params: { visionid: profileData.visionid } });
        setCourses(coursesResponse.data.courses);

        const notificationsResponse = await api.get('/loadTeNotifications', { params: { visionid: profileData.visionid } });
  ...
  const handleSeen = async (expenseid) => {
    try {
      await api.post('/markAsSeenTe', { expenseid });
            ...
            {notifications.map(notification => {
              return (
                <div key={notification.courseid} className='bg-white p-4 pb-2 mb-3 my-2 rounded flex justify-between items-center'>
                  <p>Your grade {notification.grade} - {notification.subject} class had a new expense of Rs.{notification.amount} in {notification.month}. The reason given was '{notification.reason}'.</p>
                  <button onClick={() => handleSeen(notification.expenseid)} className='bg-indigo-900 hover:bg-indigo-950 text-white px-2 py-1 rounded text-sm'>Seen</button>
                </div>
              )
            })}
          </div>
